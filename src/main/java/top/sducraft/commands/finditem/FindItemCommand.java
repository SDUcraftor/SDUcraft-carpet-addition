    ...
    private static int execute(@NotNull CommandSourceStack source, ItemInput targetItemInput, int radius) throws CommandSyntaxException {
        ...
        if (containers.isEmpty() && droppedItems.isEmpty()) {
            ...
            if (!containers.isEmpty()) {
                source.sendSuccess(() -> Component.literal("--- 找到 " + containers.size() + " 个包含").append(displayStack.getHoverName()).append("的容器 ---").withStyle(ChatFormatting.GREEN), false);
                for (ItemFinder.FoundContainerInfo info : containers) {
                    ...
                    Component message = Component.literal(String.format(" - (%.1fm) ", distance))
                            ...
                            .append(Component.literal("x" + info.itemCount()) // 物品数量
                                    .withStyle(Style.EMPTY
            ...
            if (!droppedItems.isEmpty()) {
                source.sendSuccess(() -> Component.literal("--- 找到 " + droppedItems.size() + " 个").append(displayStack.getHoverName()).append("掉落物 ---(仅显示最近15个)").withStyle(ChatFormatting.GREEN), false);
                int i = 0;
        ...
        return containers.size() + droppedItems.size();
    }
}